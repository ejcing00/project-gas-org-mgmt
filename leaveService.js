...
function LEAVE_listEmployeesForSetting(payload){
  ...
  try{
    ...
    //    - period: 개월
    // ======================================================
    var exVals = shEx.getDataRange().getValues();
...
function LEAVE_listLeaveEmployees(payload){
  ...
  try{
    ...
    (function buildEmpNameMap_(){
      ...
      if (!shE) return;
      var vr = shE.getDataRange().getValues();
    ...
    (function calcUsed_(){
      ...
      if (!shA) return;
      var vr = shA.getDataRange().getValues();
      if (!vr || vr.length < 2) return;
      var h = vr[0].map(function(x){ return String(x||'').trim(); });
      var im = indexMap_(h);
...
function LEAVE_updateLeaveApplyStatus(payload){
  ...
  try{
    ...
    for (var r=1; r<values.length; r++){
      ...
      if (id) map[id] = r + 1; // sheet row index
    }
    ...
    updates.forEach(function(u){
      ...
      // 소프트삭제 행은 무시
      if (cDel != null){
        var d = String(sh.getRange(rowIndex, cDel+1).getValue() || '').trim();
        if (d === '1' || d.toLowerCase() === 'true') return;
      }

      var st = String(u.status || '').trim() || '신청';
      sh.getRange(rowIndex, cSt+1).setValue(st);

      if (st === '승인' || st === '미승인'){
        if (cAt != null) sh.getRange(rowIndex, cAt+1).setValue(now);
        if (cBy != null) sh.getRange(rowIndex, cBy+1).setValue(actorLabel);
      }else{
        if (cAt != null) sh.getRange(rowIndex, cAt+1).setValue('');
        if (cBy != null) sh.getRange(rowIndex, cBy+1).setValue('');
      }
...
function LEAVE_softDeleteLeaveApplies(payload){
  ...
  try{
    ...
    ids.forEach(function(id){
      ...
      var rowIndex = map[id];
      if (!rowIndex) return;
      sh.getRange(rowIndex, cDel+1).setValue(1);
      deleted++;
    });
